...
**技术栈**: NestJS + FastifyAdapter + Node.js 20.x  
**部署环境**: 阿里云函数计算 (Function Compute)  
**数据库**: 阿里云 RDS MySQL 8.0

## 技术环境要求
- **Node.js**: 20.x (Debian 10)
- **框架**: NestJS + FastifyAdapter (高性能与低冷启动)
- **入口文件**: `fc-handler.ts` (阿里云 FC 调用入口)
- **数据库**: 阿里云 RDS MySQL 8.0 + 连接池 (连接数 ≤ 5)
- **构建方式**: NestCLI → dist → 上传到 FC
...
## 阿里云 FC 特定要求
- 使用 Fastify 的 `inject()` 方式适配 Serverless
- 无需 pm2 和 HTTP 服务器
- 代码必须可直接部署到阿里云 FC
- 冷启动时间控制在合理范围内
...
## 模块结构规范
```
AppModule → Controller → Service → Module → Filter/Guard/Pipe
```

## 开发任务优先级
1. ✅ 生成符合 NestJS + Fastify + 阿里云 FC 的 handler、main.ts、目录结构
2. ✅ 提供连接 MySQL 8.0 的最佳实践 (Serverless 场景)
3. ✅ 生成 Controller、Service、Module 的可运行示例
4. ✅ 确保所有代码与依赖可直接部署到阿里云 FC
